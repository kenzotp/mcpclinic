# wissen/ — Content-Kalender (Entwürfe)

## Fertig (Batch 1, 5 Posts — W2-Ziel erfüllt, ~2,5 Wochen Vorsprung)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `was-ist-mcp.md` | Was ist MCP / Model Context Protocol erklärt | Live-Test |
| `mcp-sicherheitsluecken.md` | MCP Sicherheitslücken | Live-Test → Audit |
| `mcp-server-dsgvo.md` | MCP Server DSGVO | Audit |
| `zammad-mcp.md` | Zammad MCP | Audit → Build |
| `easybill-mcp.md` | easybill MCP (Gerücht-Auflösung, Intent-Capture) | Live-Test → Build |
| `jtl-mcp.md` | JTL MCP / Build with AI | Live-Test → Audit |

*(6 statt 5 — der JTL-Winkel war zu gut, um liegen zu lassen.)*

## Fertig (Batch 2, 6 Posts — Kalender W2–W4 komplett vorlauf)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `awork-mcp.md` | awork MCP (Benchmark-Teardown, anerkennend) | Live-Test → Audit |
| `seven-io-mcp.md` | seven.io MCP (offizieller Anbieter #2) | Live-Test |
| `personio-mcp.md` | Personio MCP / llms.txt vs. Zugang | Audit → Build |
| `xentral-mcp.md` | Xentral MCP / public specs, PAT-Lücke | Live-Test → Build |
| `propstack-mcp.md` | propstack MCP (5 Community-Repos = Nachfrage) | Live-Test → Build |
| `agent-scanner-limits.md` | isitagentready / Scanner vs. Audit (Differenzierung) | Live-Test → Audit |

**Gesamtstand: 12 Posts fertig (Batch 1 + 2). Veröffentlichungstakt 2/Woche ≈ 6 Wochen Vorsprung.**

## Fertig (Batch 3, 4 Posts)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `collmex-mcp.md` | Collmex MCP (CSV-Protokoll als Anti-Muster) | Test → Build |
| `lexware-mcp.md` | Lexware MCP (31★-Teardown, OAuth-Schicht) | Audit |
| `self-host-oder-gateway.md` | Gateway vs. eigener Server (Entscheidungspiece) | Test → Build |
| `mcp-spezifikation-juli-2026.md` | Spec 2026-07-28 erklärt (Retainer-Hook) | Test → Retainer |

**Gesamtstand: 16 Posts.**

## Fertig (Batch 4, 4 Posts — veröffentlicht 21.09.2026)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `was-kostet-ein-mcp-server.md` | MCP Server Kosten / was kostet ein MCP-Server | Audit → Build |
| `mcp-api-key-oauth.md` | API Key vs OAuth / MCP OAuth 2.0 | Test → Audit |
| `mcp-vs-rest-api.md` | MCP vs REST API | Test → Build |
| `mcp-tool-beschreibungen.md` | MCP Tool Description / Tool-Beschreibungen schreiben | Audit |

**Gesamtstand: 20 Posts.**

## Fertig (Batch 5, 3 Posts — veröffentlicht 22.09.2026)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `remote-mcp-server.md` | Remote MCP Server (DE-Autocomplete 22.09.: eigener Baum mit claude, claude desktop, oauth, url) | Live-Test |
| `mcp-oekosystem.md` | MCP Server Deutschland / MCP Server Liste | Report |
| `clockodo-mcp.md` | Clockodo MCP / Clockodo MCP-Server | Live-Test |

**Gesamtstand: 23 Posts.**

## Fertig (Batch 6, 2 Posts — veröffentlicht 23.09.2026)

| Datei | Ziel-Keyword | CTA |
|---|---|---|
| `mcp-faehig-machen.md` | Software MCP-fähig machen / agentenfähig machen | Live-Test |
| `mcp-agentur.md` | MCP-Agentur / MCP-Beratung / MCP Consulting | Live-Test → Audit |

**Gesamtstand: 25 Posts.**

## Redaktionsregeln (aus dem Playbook + Report-Prinzipien)

- Jeder Post endet mit genau einem CTA-Pfad (Live-Test ODER Audit ODER Build), nie allen dreien gleichwertig.
- Zahlen nur aus dem eigenen Report (nicht erfinden; wenn externe Zahlen: Quelle verlinken).
- Hersteller nie angreifend schreiben — wir wollen sie als Audit-Kunden, nicht als Gegner.
- llms.txt nur als „kosmetisch" bewerten (Google-Studie 6/2026), nie als Verkaufsargument.
- Fachbegriffe beim ersten Auftreten in einem Satz erklären (Zielgruppe: Gründer/Geschäftsführung, nicht nur Devs).
