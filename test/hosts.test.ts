...
test("alternate hosts: apex and www entries widen the net", () => {
  assert.deepEqual(deriveAlternateHosts("lexware.de"), ["docs.lexware.de", "api.lexware.de"]);
  assert.deepEqual(deriveAlternateHosts("www.lexware.de"), ["docs.lexware.de", "api.lexware.de"]);
});
...
test("alternate hosts: never on IPs or garbage", () => {
  ...
  assert.deepEqual(deriveAlternateHosts("example.invalid"), ["docs.example.invalid", "api.example.invalid"]);
});
